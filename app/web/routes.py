...
def create_router(settings: Settings) -> APIRouter:
    ...
    async def system_page(request: Request) -> HTMLResponse:
        ...
        selected_ids = await selection_repository.effective_known_ids(settings.configured_chat_ids)
        storage = await asyncio.to_thread(inspect_storage, settings, completed_paths)
        context = _navigation_context(request, "system")
        context.update(
            {
                "safe_settings": {
                    "Database": settings.database_url,
                    "Download directory": str(settings.download_dir),
                    "Selected chats": len(selected_ids),
                    "Maximum file size": f"{settings.max_file_size_mb} MB",
                    "Download concurrency": settings.download_concurrency,
                    "Allowed extensions": settings.allowed_extensions or "All except ignored",
                    "Ignored extensions": settings.ignored_extensions or "None",
                    "Keyword filter": "Enabled" if settings.keyword_set else "Disabled",
                    "Log level": settings.log_level,
                },
                "session_exists": await asyncio.to_thread(session_path.is_file),
                "web_auth_enabled": bool(
                    settings.web_password and settings.web_password.get_secret_value()
                ),
                "loopback_only": _loopback_host(settings.web_host),
                "storage": storage,
            }
        )
