...
class ArchiveRepository:
    ...
    async def upsert_message(self, data: MessageData) -> tuple[MessageSnapshot, bool]:
        async with self.database.sessions() as session, session.begin():
            ...
            initial_status = "pending" if data.has_media else "not_applicable"
            if record is None:
                record = Message(
            ...
            if not data.has_media:
                record.download_status = "not_applicable"
    ...
    async def mark_download_skipped(self, message_id: int, reason: str) -> None:
        async with self.database.sessions() as session, session.begin():
            await session.execute(
                update(Message)
                .where(Message.id == message_id)
                .values(download_status="skipped", download_error=reason, updated_at=utc_now())
            )
    ...
    async def mark_download_failed(self, message_id: int, error: str) -> None:
        async with self.database.sessions() as session, session.begin():
            await session.execute(
                update(Message)
                .where(Message.id == message_id)
                .values(download_status="failed", download_error=error[:4000], updated_at=utc_now())
            )
    ...
    async def iter_retry_candidates(
        ...
        *,
        ...
        if not chat_ids:
            return []
        statuses = ("failed",) if failed_only else ("pending", "downloading", "failed", "completed")
        ...
        async with self.database.sessions() as session:
            ...
            downloaded_files = int(
                await session.scalar(
                    select(func.count(Message.id)).where(Message.download_status == "completed")
                )
                or 0
            )
            downloaded_bytes = int(
            ...
            failed = int(
                await session.scalar(
                    select(func.count(Message.id)).where(Message.download_status == "failed")
                )
                or 0
            )
            skipped = int(
                await session.scalar(
                    select(func.count(Message.id)).where(Message.download_status == "skipped")
                )
                or 0
            )
