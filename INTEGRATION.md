...
The application already uses Telegram's MTProto API directly. Optional proxy discovery uses MTPro.XYZ when `TG_MTPROTO_PROXY_AUTO=true`, applies a selected proxy to every Telethon client including media downloads, and rotates candidates when sufficiently large transfers remain below 1,000,000 bytes/s. This is an enforced best-effort floor, not a guarantee against external network conditions or Telegram rate limits.
